...
# Copied from ppdiffusers.pipelines.stable_diffusion_3.pipeline_stable_diffusion_3.StableDiffusion3Pipeline.retrieve_timesteps
...
class StableDiffusion3ControlNetInpaintingPipeline(DiffusionPipeline, SD3LoraLoaderMixin, FromSingleFileMixin):
    ...
    # Copied from ppdiffusers.pipelines.stable_diffusion_3.pipeline_stable_diffusion_3.StableDiffusion3Pipeline._get_t5_prompt_embeds
    ...
    # Copied from ppdiffusers.pipelines.stable_diffusion_3.pipeline_stable_diffusion_3.StableDiffusion3Pipeline._get_clip_prompt_embeds
    ...
    # Copied from ppdiffusers.pipelines.stable_diffusion_3.pipeline_stable_diffusion_3.StableDiffusion3Pipeline.prepare_latents
    def prepare_latents(
        self,
        batch_size,
    ...
    # Modify from diffusers.pipelines.stable_diffusion_3.pipeline_stable_diffusion_3_controlnet_inpainting.StableDiffusion3ControlNetInpaintingPipeline.prepare_image_with_mask
    def prepare_image_with_mask(
        ...
        mask = mask.repeat_interleave(repeat_by, axis=0)
    ...
    def __call__(
        ...
        controlnet_pooled_projections: Optional[paddle.Tensor] = None,
        negative_prompt: Optional[Union[str, List[str]]] = None,
        negative_prompt_2: Optional[Union[str, List[str]]] = None,
        negative_prompt_3: Optional[Union[str, List[str]]] = None,
        num_images_per_prompt: Optional[int] = 1,
        generator: Optional[Union[paddle.Generator, List[paddle.Tensor]]] = None,
        ...
        Args:
            ...
            control_image (`paddle.Tensor`, `PIL.Image.Image`, `np.ndarray`, `List[paddle.Tensor]`, `List[PIL.Image.Image]`, `List[np.ndarray]`):
                ...
                paddle tensor, the expected value range is between `[0, 1]` If it's a tensor or a list or tensors, the
                expected shape should be `(B, C, H, W)`. If it is a numpy array or a list of arrays, the expected shape
                should be `(B, H, W, C)` or `(H, W, C)`.
            control_mask (`paddle.Tensor`, `PIL.Image.Image`, `np.ndarray`, `List[paddle.Tensor]`, `List[PIL.Image.Image]`, `List[np.ndarray]`):
                ...
                color channel (L) instead of 3, so the expected shape for paddle tensor would be `(B, 1, H, W)`. And
                for numpy array would be for `(B, H, W, 1)`, `(B, H, W)`, `(H, W, 1)`, or `(H, W)`.
            controlnet_conditioning_scale (`float` or `List[float]`, *optional*, defaults to 1.0):
                The outputs of the ControlNet are multiplied by `controlnet_conditioning_scale` before they are added
                to the residual in the original `unet`. If multiple ControlNets are specified in `init`, you can set
                the corresponding scale as a list.
            controlnet_pooled_projections (`paddle.Tensor` of shape `(batch_size, projection_dim)`):
        ...
        if controlnet_pooled_projections is None:
            controlnet_pooled_projections = paddle.zeros_like(pooled_prompt_embeds)
        else:
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # TODO: is_inference_mode
                # TODO: inference_optimize_bp
